...
impl ComposeExecutorImpl {
    ...
    fn build_cmd(&self, engine: &str, machine_name: Option<&str>) -> Command {
...
impl ReconcilerPort for ComposeExecutorImpl {
        ...
        Box::pin(async move {
            ...
            if let (Some(host), Some(pass)) = (reg_host.as_ref(), reg_pass.as_ref()) {
                if !host.trim().is_empty() && !pass.trim().is_empty() {
                    ...
                    let mut login_cmd = self.build_cmd(&engine, config.machine_name.as_deref());
            ...
            let engine = self.engine().await;
            let mut output_cmd = self.build_cmd(&engine, config.machine_name.as_deref());
    ...
    fn apply<'a>(
        ...
        let path_buf = compose_path.to_path_buf();
        let machine_name_owned = machine_name.map(String::from);

        Box::pin(async move {
            let engine = self.engine().await;
            let mut output_cmd = self.build_cmd(&engine, machine_name_owned.as_deref());
            let output = output_cmd
                .args([
                    "compose",
                    "-f",
    ...
    fn is_running<'a>(
        ...
        Box::pin(async move {
            ...
            let mut command = self.build_cmd(&engine, machine_name_owned.as_deref());
            command.args([
                "compose",
                "-p",
                &stack_id_str,
