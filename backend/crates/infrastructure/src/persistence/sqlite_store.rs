...
impl StateStore for SqliteStateStore {
        ...
        Box::pin(async move {
            ...
            sqlx::query(
                ...
                INSERT INTO stack_config
                    (id, repo_url, branch, compose_path, poll_interval_secs, pat_token, env_vars, registry_host, registry_user, registry_pass, sync_mode, webhook_secret, is_protected, security_pin, source_type, inline_compose)
                VALUES
    ...
    fn load_all_stack_configs(
        ...
        Box::pin(async move {
            let rows = sqlx::query(
                r#"
                SELECT id, repo_url, branch, compose_path, poll_interval_secs, pat_token, env_vars, registry_host, registry_user, registry_pass, sync_mode, webhook_secret, is_protected, security_pin, source_type, inline_compose
                FROM stack_config
                "#,
            )
            .fetch_all(&self.pool)
    ...
    fn save_settings<'a>(
        ...
        Box::pin(async move {
            sqlx::query(
                r#"
                INSERT INTO settings (key, value) VALUES ('container_engine', ?1)
    ...
    fn load_settings(&self) -> BoxFuture<'_, Result<domain::entities::settings::AppSettings, StoreError>> {
        Box::pin(async move {
            let row = sqlx::query("SELECT value FROM settings WHERE key = 'container_engine'")
                .fetch_optional(&self.pool)
                .await
                .map_err(|e| StoreError::Database(e.to_string()))?;

            let container_engine = if let Some(r) = row {
                r.get("value")
            } else {
                "podman".to_string()
            };

            Ok(domain::entities::settings::AppSettings { container_engine })
        })
    }
