...
pub async fn start_machine(
    ...
    match res {
        Ok(out) if out.status.success() => {
            // Set as default connection after starting
            let _ = Command::new("podman").args(["system", "connection", "default", &name]).output().await;
            (StatusCode::OK, Json(json!({"message": format!("Machine '{}' started and set as default", name)}))).into_response()
        },
        Ok(out) => {
            let stderr = String::from_utf8_lossy(&out.stderr);
