...
async fn process_webhook_trigger(state: &AppState, secret: &str) -> axum::response::Response {
    ...
    match uc.execute(stack, EventKind::WebhookSync).await {
        Ok(updated) => (
            StatusCode::OK,
            Json(json!({
                "message": format!("Webhook payload processed successfully. Auto-deployment triggered for stack '{}'", stack_id),
                "stack": updated
            })),
        )
            .into_response(),
        Err(e) => (
            ...
            Json(json!({ "error": format!("Webhook auto-deployment failed: {e}") })),
        )
            .into_response(),
    }
}
