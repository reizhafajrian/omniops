            ...
            _ => {
                ...
                if hash_changed {
                    ...
                    if stack.config.source_type == "inline" {
                        let dir = std::env::temp_dir().join("omniops_inline").join(stack.config.id.as_str());
                        if dir.exists() {
                            dir_buf = dir;
                    ...
                    let running = self.reconciler.is_running(&stack.config.id, working_dir, stack.config.machine_name.as_deref()).await.unwrap_or(true);
                    !running
                }
            }
        };
