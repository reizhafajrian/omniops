...
import { Layers, Key, RefreshCw, Cpu, LogOut, Plus } from 'lucide-react';
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Self-Hosted GitOps Control Plane for Docker Compose Stacks
              </p>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-dark-900 border border-slate-800 text-xs font-medium text-slate-300 hover:text-white hover:border-slate-700 transition-colors"
            >
              <Key size={14} className={token ? 'text-emerald-400' : 'text-amber-400'} />
              <span>{token ? 'Auth Token Set' : 'Configure Token'}</span>
            </button>
      ...
      <SettingsModal 
        ...
        onClose={() => {
          ...
          onRefresh(); // Refresh stacks and status after engine change
        }}
      />
    </>
  );
