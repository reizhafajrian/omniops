...
import { ChevronDown, ExternalLink, Unlock, Container, Github, Cloud, Box, Settings } from 'lucide-react';
...
export const RegistryAuthSection: React.FC<RegistryAuthSectionProps> = ({ value, onChange }) => {
  ...
  useEffect(() => {
    if (!value.registry_host) {
      setSelectedProvider('none');
    } else if (value.registry_host.includes('gcr.io')) {
      setSelectedProvider('gcr');
    } else if (value.registry_host.includes('pkg.dev')) {
      setSelectedProvider('gar');
    } else if (value.registry_host === 'ghcr.io') {
      setSelectedProvider('ghcr');
    } else if (value.registry_host.includes('amazonaws.com')) {
      setSelectedProvider('ecr');
    } else if (value.registry_host === 'docker.io') {
      ...
      <div className="relative">
        ...
        >
          <div className="flex items-center gap-2.5">
            <span className={clsx('text-sm', provider.logoColor)}>{provider.icon}</span>
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-1 z-30 bg-dark-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
            ...
            >
              <span className="text-slate-500"><Unlock size={14} /></span>
            ...
            <div className="max-h-52 overflow-y-auto">
              {nonNoneProviders.map((p) => (
                ...
                >
                  <span className={clsx('text-sm w-5 text-center flex justify-center', p.logoColor)}>{p.icon}</span>
