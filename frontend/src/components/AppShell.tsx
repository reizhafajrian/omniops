...
interface AppShellProps {
  children: (activeTab: 'stacks' | 'docker' | 'podman', onNavigateToEngine: (engine: string) => void) => React.ReactNode;
  onLogout?: () => void;
  onAddStack?: () => void;
  onRefresh: () => void;
...
export const AppShell: React.FC<AppShellProps> = ({
  ...
  const getInitialSettings = () => {
    ...
    if (hashSplit.length > 1) {
      const params = new URLSearchParams(hashSplit[1]);
      return params.get('modal') === 'settings';
    }
    return false;
  };
  ...
  const [isSettingsOpen, setIsSettingsOpen] = useState(getInitialSettings());
  ...
  const updateUrlParams = useCallback((updates: Record<string, string | null>, newPath?: string) => {
    ...
    if (changed) {
      ...
      // Trigger a hashchange event manually so the rest of the app updates if needed
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (settings !== isSettingsOpen) setIsSettingsOpen(settings);
    };
  ...
  const handleSettingsChange = (isOpen: boolean) => {
    setIsSettingsOpen(isOpen);
    updateUrlParams({ modal: isOpen ? 'settings' : null });
  };
    ...
    <TooltipProvider>
      <SidebarProvider>
        <AppSidebar
          ...
          onOpenSettings={() => handleSettingsChange(true)}
        ...
        <SidebarInset className="flex flex-col min-h-screen">
          ...
          <main className="flex-1 overflow-auto p-6" id="main-content">
            {children(activeTab, handleNavigateToEngine)}
          </main>
        </SidebarInset>
