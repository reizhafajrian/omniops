...
import { Layers, Server, Cpu, Settings, RefreshCw, LogOut, Plus } from 'lucide-react';
...
export const AppSidebar: React.FC<AppSidebarProps> = ({
  ...
  const navItems = [
    ...
    },
    {
      ...
      label: 'Docker',
      icon: Server,
      badge: { label: 'Disabled', variant: 'outline' as const },
      disabled: true,
    },
    {
      id: 'podman' as const,
    ...
    <Sidebar>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-base font-bold tracking-tight text-sidebar-foreground">OmniOps</span>
              <Badge variant="outline" className="text-[9px] px-1.5 py-0 font-mono border-primary/30 text-primary">
                Control Plane
              </Badge>
            </div>
            <p className="text-[10px] text-muted-foreground leading-tight mt-0.5">OmniOps Container Platform</p>
          </div>
        </div>
      </SidebarHeader>
      ...
      <SidebarContent>
        <SidebarGroup>
          ...
          <SidebarMenu>
            {navItems.map(({ id, label, icon: Icon, badge, disabled }) => (
              <SidebarMenuItem key={id}>
                <SidebarMenuButton
                  ...
                  className={cn("justify-between", disabled && "opacity-50 cursor-not-allowed")}
                >
                  ...
                  {badge && (
                    <Badge variant={badge.variant} className={cn("text-[10px] px-1.5 py-0 h-4 ml-auto", disabled && "text-slate-400 border-slate-700")}>
                      {badge.label}
                    </Badge>
                  )}
                </SidebarMenuButton>
