...
import { 
  ...
  Loader2, AlertCircle, AlertTriangle, HardDrive, Cpu, MemoryStick
} from 'lucide-react';
...
export const PodmanManagerTab: React.FC = () => {
  ...
  const [deleteMachineName, setDeleteMachineName] = useState<string | null>(null);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['podman', 'machines'] });
      setErrorMsg(null);
      setDeleteMachineName(null);
    },
    onError: (err: any) => {
      setErrorMsg(err.message || 'Failed to delete machine');
      setDeleteMachineName(null);
    }
  });
  ...
  return (
    <div className="flex flex-col h-full">
      <AlertDialog open={!!deleteMachineName} onOpenChange={(open) => !open && setDeleteMachineName(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete machine <strong>&apos;{deleteMachineName}&apos;</strong>?
              This will destroy all containers and images inside it. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (deleteMachineName) {
                  deleteMutation.mutate(deleteMachineName);
                }
              }}
            >
              Delete Machine
            ...
            <div className="space-y-4">
              ...
              {isCreating && (
                <div className="bg-dark-900 border border-brand-500/30 rounded-xl p-4 mb-4">
                  ...
                  <div className="mb-4 bg-brand-500/10 border border-brand-500/20 rounded-lg p-3 text-brand-300 text-xs flex items-start gap-2">
                    <AlertCircle size={14} className="shrink-0 mt-0.5" />
                    <p>Creating a new machine will automatically stop any currently running machines and start the new one.</p>
                  </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setDeleteMachineName(machine.Name);
                      }}
